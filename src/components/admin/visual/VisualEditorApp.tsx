...
import { AdminContentProvider, useAdminContent } from "@/components/admin/AdminContentContext";
...
function VisualEditorWorkspace({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    if (!projectSlug || !content.projects.some((project) => project.slug === projectSlug)) {
    ...
    <MediaPickerProvider>
      ...
      >
        <div className="min-h-screen bg-background">
          <VisualEditorWorkspace onLogout={() => void handleLogout()} />
        </div>
      </AdminContentProvider>
    </MediaPickerProvider>
  );
